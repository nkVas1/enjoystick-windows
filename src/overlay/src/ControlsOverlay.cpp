...
void ControlsOverlay::BuildSections() {
    ...
    }
    {
        ...
        s.bindings = {
            ...
            { L"Right Stick",              L"Proximity hover (swipe)" },
            { L"\u25CF  (A)",             L"Type key  (hold: accelerates)" },
            { L"\u25A0  (X)",             L"Backspace  (hold: accelerates)" },
...
void ControlsOverlay::Open() {
    ...
    m_panelSpring.stiffness = 320.0f; m_panelSpring.damping = 26.0f;
    m_panelSpring.Snap(0.0f); m_panelSpring.SetTarget(1.0f);

    m_tabSpring.stiffness = 400.0f; m_tabSpring.damping = 28.0f;
    m_tabSpring.Snap(0.0f); m_tabSpring.SetTarget(0.0f);
}
...
void ControlsOverlay::Update(const ControllerState& state, float dt) {
    ...
    if (m_state == State::Closing) {
        if (m_panelSpring.IsSettled(0.01f) || pv <= 0.001f) { m_state = State::Hidden; return; }
    }
    if (m_state == State::Hidden) return;

    m_tabSpring.Step(dt);

    if (m_openMask == Button::None) m_openMask = state.buttons;
    m_openMask = static_cast<Button>(
        static_cast<uint32_t>(m_openMask) & static_cast<uint32_t>(state.buttons));

    const uint32_t effectiveMask =
        static_cast<uint32_t>(state.buttons) & ~static_cast<uint32_t>(m_openMask);
    auto btn = [&](Button b) -> bool {
        return (effectiveMask & static_cast<uint32_t>(b)) != 0;
    };

    const bool east   = btn(Button::East);
    const bool south  = btn(Button::South);  // kept for prev tracking only
    ...
    // Only East (B) closes — South (A) intentionally NOT closing this overlay.
    ...
    if (dRight && !m_prevDRight) {
        ...
        m_stickActive = false; m_scrollOffset = 0;
        m_scrollDpadHeld = false; m_scrollRyActive = false;
    ...
    // ---- Section navigation (Left Stick X)
    {
        ...
        if (std::abs(lx) > 0.50f) {
            ...
            m_stickActive = false; m_stickCooldown = 0.0f;
        }
    }
    ...
    // ---- Scroll binding list (DPad Up / Down)
    {
        ...
        if (dVert) {
            if (!m_scrollDpadHeld) {
                ...
                if (dUp && m_scrollOffset > 0) { --m_scrollOffset; if (m_onHaptic) m_onHaptic(HapticType::Nav); }
                if (dDown) {
                    ...
                    if (m_scrollOffset < total - 1) { ++m_scrollOffset; if (m_onHaptic) m_onHaptic(HapticType::Nav); }
                }
            } else {
            ...
            m_scrollDpadHeld = false; m_scrollDpadTimer = 0.0f;
        }
    }
    ...
    // ---- Scroll binding list (Right Stick Y)
    {
        ...
        if (std::abs(ry) > kScrollRyDz) {
            if (!m_scrollRyActive) {
                ...
                const int32_t total = m_sectionIdx < static_cast<int32_t>(m_sections.size())
                    ? static_cast<int32_t>(m_sections[static_cast<size_t>(m_sectionIdx)].bindings.size()) : 0;
                if (ry < 0 && m_scrollOffset > 0)       { --m_scrollOffset; if (m_onHaptic) m_onHaptic(HapticType::Nav); }
                if (ry > 0 && m_scrollOffset < total-1) { ++m_scrollOffset; if (m_onHaptic) m_onHaptic(HapticType::Nav); }
            } else {
            ...
            m_scrollRyActive = false; m_scrollRyCooldown = 0.0f;
        }
    }
...
static void DrawChip(
    ...
    float x, float y, float s, float ease,
    const wchar_t* text, bool isKey, float fontSize) noexcept
{
    if (!rt || !dwrite) return;
    ...
    if (isKey) {
        ...
        const float cw = tm.width+pH*2.0f, ch = tm.height+pV*2.0f;
    ...
    float  screenH) const
{
    ...
    { Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> b;
      ...
      if (b) { rt->FillRectangle(D2D1::RectF(panelX,panelY+accH,panelX+panelW,panelY+accH+cr),b.Get());
               D2D1_ROUNDED_RECT rr{D2D1::RectF(panelX,panelY+accH,panelX+panelW,panelY+panelH),cr,cr};
               rt->FillRoundedRectangle(rr,b.Get()); } }
    { Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> b;
    ...
    { Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> b;
      ...
      if (b) rt->DrawLine(D2D1::Point2F(panelX+padX,divY),D2D1::Point2F(panelX+panelW-padX,divY),b.Get(),0.8f); }
        ...
        for (int32_t i = safeOffset; i < total; ++i) {
            ...
            if ((i-safeOffset)%2==0) {
                ...
                if (bg) { D2D1_ROUNDED_RECT rr{
                    ...
                    4.0f*s,4.0f*s}; rt->FillRoundedRectangle(rr,bg.Get()); }
            }
            const float chipH = 22.0f*s, chipVOff = (rowH-chipH)*0.5f;
            ...
            DrawChip(rt,dwrite, actX, ry+(rowH-13.5f*s-4.0f*s)*0.5f, s, ease, bind.action.c_str(), false, fntSize);
        }
        ...
        for (int32_t i = 0; i < n; ++i) {
            ...
            if (dwrite) {
                ...
                if (fmt) {
                    ...
                    const float alpha = isSel ? (0.60f+0.40f*blend)*ease : 0.38f*ease;
                    Microsoft::WRL::ComPtr<ID2D1SolidColorBrush> b;
                    rt->CreateSolidColorBrush(
                        isSel ? Tok::GoldHi(alpha) : Tok::ChromeMute(alpha), b.GetAddressOf());
                    if (b) rt->DrawText(icon.c_str(), static_cast<UINT32>(icon.size()),
                        fmt.Get(), D2D1::RectF(tx,tabBarY+2.0f*s,tx+tabW,tabBarY+42.0f*s), b.Get());
                }
            }
            if (isSel) {
        ...
        if (dwrite) {
            ...
            if (fmt) {
                ...
                if (b) rt->DrawText(nh, static_cast<UINT32>(std::wcslen(nh)),
                    fmt.Get(),
                    D2D1::RectF(panelX+padX,tabBarY,panelX+panelW-padX-4.0f*s,tabBarY+42.0f*s),
                    b.Get());
            }
        }
    }
