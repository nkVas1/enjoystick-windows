...
.DESCRIPTION
    ...
    vcpkg integration: user-level (no system-wide hook).
...
# Set VCPKG_ROOT in user environment (survives reboots)
...
if (-not (Test-Path $NinjaExe)) {
    ...
    # Fetch latest Ninja release asset URL
    ...
    if (-not $Asset) {
        Write-Host "    [WARN] Could not auto-download Ninja. Download manually from https://ninja-build.org/" -ForegroundColor Yellow
    } else {
...
# Done
# ---------------------------------------------------------------------------
Write-Host ""`n✅ Dependencies ready. Open a NEW terminal for PATH changes to take effect."" -ForegroundColor Green
